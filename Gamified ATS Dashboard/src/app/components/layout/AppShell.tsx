...
export function AppShell({ children }: { children: React.ReactNode }) {
  const { persona, currentApplicant, currentTeamMember, setPersona } = useApp();
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#0a0a14', color: '#f1f5f9' }}>
      ...
      >
        ...
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            const isActive = location.pathname === item.path || (item.path !== '/applicant' && item.path !== '/recruiter' && item.path !== '/interviewer' && location.pathname.startsWith(item.path));
            return (
              <Link
                key={item.path}
                to={item.path}
