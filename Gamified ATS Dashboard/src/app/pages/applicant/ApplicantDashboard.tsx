...
import {
  getJobById, getCompanyById, allTasks, Task,
...
export default function ApplicantDashboard() {
  const { currentApplicant, allJobs, applyToJob, completeTask } = useApp();
  ...
  const handleApply = () => {
    ...
    if (!job) return;
    applyToJob(selectedJob, job.companyId, source || 'Direct');
    setApplyModal(false);
    setApplyStep(1);
    setSelectedJob(null);
    setSource('');
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {activeApplications.length === 0 ? (
            ...
            activeApplications.map((app) => (
              <ApplicationCard key={app.id} app={app} onCompleteTask={handleCompleteTask} completedTaskIds={completedTaskIds} />
            ))
          )}
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'rgba(124,58,237,0.15)', background: 'rgba(255,255,255,0.01)' }}>
            ...
            <div className="p-3 space-y-2">
              ...
              <button
                onClick={() => navigate('/applicant/tasks')}
