...
export default function RecruiterKanban() {
  ...
  const { allJobs, allApplicants, updateApplicationStage, updateApplicantFeedback, currentTeamMember } = useApp();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0a0a14' }}>
        <div className="text-center">
          ...
          <button onClick={() => navigate('/recruiter/jobs')} style={{ color: '#a78bfa', fontSize: 13, marginTop: 8 }}>← Back</button>
        </div>
      </div>
    );
  }
  ...
  const handleAdvance = (applicant: Applicant, application: Application) => {
    ...
    if (nextStage && nextStage !== 'rejected') {
      updateApplicationStage(applicant.id, application.id, nextStage);
    }
  };

  const handleReject = (applicant: Applicant, application: Application) => {
    updateApplicationStage(applicant.id, application.id, 'rejected');
    setSelectedCard(null);
  };

  const handleSaveFeedback = () => {
    if (!selectedCard) return;
    updateApplicantFeedback(selectedCard.applicant.id, selectedCard.application.id, feedbackDraft);
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#0a0a14' }}>
      ...
      <div className="px-6 pt-8 pb-5 border-b shrink-0" style={{ borderColor: 'rgba(124,58,237,0.1)' }}>
        <button onClick={() => navigate('/recruiter/jobs')} className="flex items-center gap-1.5 mb-3" style={{ fontSize: 13, color: '#64748b' }}>
