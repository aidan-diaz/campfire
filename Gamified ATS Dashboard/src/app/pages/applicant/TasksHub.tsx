...
import { allTasks, Task, getDifficultyConfig, getCompanyById, getJobById } from '../../data/mockData';
...
export default function TasksHub() {
  const { currentApplicant, completeTask } = useApp();
  ...
  const filtered = allTasks.filter((t) => {
    ...
    if (typeFilter !== 'completed') return true;
    return true;
  });
...
function TaskGroup({
  title, subtitle, tasks, completedIds, onComplete, color, showCompany = false, showRole = false,
}: {
  title: string;
  subtitle: string;
  tasks: Task[];
    ...
    <div>
      ...
      <div className="space-y-2">
        {tasks.map((task) => {
          ...
          const job = showRole && task.jobId ? getJobById(task.jobId) : null;
          return (
            <div key={task.id}>
              {(company || job) && (
