...
export default function CandidateScorecard() {
  const { applicantId, jobId } = useParams<{ applicantId: string; jobId: string }>();
  const navigate = useNavigate();
  const { currentTeamMember, allApplicants, allJobs, addScorecard, scorecardsList, updateApplicationStage, updateApplicantFeedback } = useApp();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0a0a14' }}>
        <div className="text-center">
          ...
          <button onClick={() => navigate('/interviewer')} style={{ color: '#a78bfa', fontSize: 13, marginTop: 8 }}>← Back</button>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = () => {
    ...
    if (applicantFeedback) {
      updateApplicantFeedback(applicant.id, application.id, applicantFeedback);
    }
    setSubmitted(true);
  };
  ...
  const handleAdvance = () => {
    ...
    if (currentIdx < stageSeq.length - 1) {
      updateApplicationStage(applicant.id, application.id, stageSeq[currentIdx + 1]);
    }
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0a0a14' }}>
      ...
      <div className="px-6 pt-8 pb-5 border-b" style={{ borderColor: 'rgba(124,58,237,0.1)' }}>
        <button onClick={() => navigate('/interviewer')} className="flex items-center gap-1.5 mb-3" style={{ fontSize: 13, color: '#64748b' }}>
